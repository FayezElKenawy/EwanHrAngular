...
@Component({
  selector: 'app-labor-replacement',
  templateUrl: './labor-replacement.component.html',
  styleUrls: ['./labor-replacement.component.scss']
})
export class LaborReplacementComponent implements OnInit {
  ...
  replaceLaborer(){ 
    ...
    postedVM.expectedStartDate= this._datePipe.transform(this.form.get('ExpectedContractDate').value);
    postedVM.newLaborFileId= this.selectedLaborer.Id;
    debugger;
    postedVM.laborerStatusId = this.form.get('LaborerStatusId').value.Id

    this._contractService.replaceLabor(postedVM).subscribe(res=>{
      if(res.isSuccess){
